@extends('layouts.assistant')
@section('page-title','Modificar modelo')
@section('contenido')
<div class="row">

    @include('assistant.models.fragment.alert')
        <div class="row">&nbsp;</div>
    {!! Form::model($model, ['route'=> ['models.update',$model->id], 'method'=> 'PUT']) !!}
        @include('assistant.models.fragment.formModificar')
    {!! Form::close() !!}

</div>
<div class="col-s5">
    <a class="waves-effect waves-light btn" href="{{ route('models.index') }}">Ver especialidades</a>
</div>
@if(count($errors))
        @foreach($errors->all() as $error)
        @if($error == "El campo Nombre es obligatorio.")
            <script>
                lbltxt = document.getElementById("helName");
                lbltxt.innerHTML = "El campo Nombre es obligatorio.";
                lbltxt.setAttribute("class","helper-text red-text text-darken-2");
            </script>
        @endif
        @if($error == "El campo Marca es obligatorio.")
        <script>
                lbltxt = document.getElementById("helBrand");
                lbltxt.innerHTML = "Selecciona una marca!!";
                lbltxt.setAttribute("class","helper-text red-text text-darken-2");
        </script>
        @endif
        @endforeach
@endif
@endsection